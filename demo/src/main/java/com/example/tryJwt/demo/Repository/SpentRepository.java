...
public interface SpentRepository extends CrudRepository<Spent,Integer> {
    ...
    @Query(value = "select * from spent  where id_autor = ?1 order by id_autor desc",nativeQuery = true)
    public List<Spent> findAllByUsuario(Integer id);
    //Busquda con filtro monto solamente
    @Query(value = "select * from spent where id_autor  = ?1 and monto between ?2 and ?3 order by id_autor desc", nativeQuery = true)
    public List<Spent> findAllByUsuario(Integer id,Double monto_min, Double monto_max);
    //Busqueda con filtro monto y tipo
    @Query(value = "select * from spent where id_autor  = ?1  and tipo = ?2 order by id_autor desc", nativeQuery = true)
    public List<Spent> findAllByUsuario(Integer id, String tipo);
    @Query(value = "select * from spent where id_autor  = ?1  and fecha between ?2 and ?3 order by id_autor desc", nativeQuery = true)
